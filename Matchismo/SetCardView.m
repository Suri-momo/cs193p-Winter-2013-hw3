...
- (void)drawShape: (CGRect)rect
{
    ...
    if (self.number == 0) // draw in the middle - VIOLATION of MVC - Why should view know model is zero based
        [shapeHeightDelta addObject:[NSNumber numberWithFloat:0.0f]];
    
    if (self.number == 1) { // draw on either end of the center
        [shapeHeightDelta addObject:[NSNumber numberWithFloat:-[self halfShapeHeight]*SYMBOL_GAP]];
        [shapeHeightDelta addObject:[NSNumber numberWithFloat:+[self halfShapeHeight]*SYMBOL_GAP]];
    }
    
    if (self.number == 2) { // draw on either side of the center plus the center
    ...
    for (NSNumber *num in shapeHeightDelta) {
        ...
        // the shape
        if (self.shape == DIAMOND) [self drawDiamondAtPoint:centerPoint];
        if (self.shape == SQUIGGLE) [self drawSquiggleAtPoint:centerPoint];
        if (self.shape == OVAL) [self drawOvalAtPoint:centerPoint];
    }
}
